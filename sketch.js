/* Add sound
Probably, the most difficult thing of adding sounds to the game was finding the right sounds.
It is not just about where to find the sounds, but to find a set of coherent sounds that sound
like part of the same thing.
Plugging the sounds into the game has not been too difficult.
The only issue i've had was while I was trying to play a sound when the player dies. I wanted the game to wait for
the music to finish before proceeding to restart the level and I based my code upon the result of the function
isPlaying of the sound object. Tha function was supposed to return a boolean saying whether the sound is still playing or
no more. In that particular occasion it always returned true, even when the sound was finished.
I found that I was not the only one facing that issue as this page documents:
/https://github.com/processing/p5.js-sound/issues/59
I solved the problem with a workaround.
The strange thing is that I use the same function applied to the soundtrack when I restart the level and it works fine.
I had never worked on a game before this project therefore everything I learnt about how to plug graphics and sounds
together to make a game was new to me. Very interesting.
 */

/* Create platforms
I have created the platform like made of square blocks. I can set the position, height and weight of the platform, as well
as the number of blocks it is made. I've done so because it looks like "familiar" in a platform game (I'm thinking to Mario bros).
One challenge I had to manage was make the player stay on the platform instead of falling down. I've addressed that the same
way I've done for the canyons. The player has an additional variable which is its ground level. This value is normally equal
to the ground, but it can be reset to a different value from entities like platforms (which set it higher) or canyons (which
set it lower). The logic that makes the player fall down stops when the player height has reached the value of that property.
As a further improvement, I would like to add movement to the platform. This is not apparently too difficult, but only apparently.
I guess the major difficulty would be the interaction with the player when the player is over the platform. In that case
the player should be dragged by the platform and move together with it. Again, a lot to think about and a lot of things
learnt.
 */

/* Create enemies
Not to difficult to create the enemies or their interaction with the player. However I was thinking about how to make the
enemies share common features. The answer I found is to create a base class Enemy and other classes inheriting from that
base class, specialising different types of enemies. This is object oriented programming and, yes, I am a professional developer
and already know some object oriented programming, but javascript is not a language I use on a daily basis and, as far as
I understand, oop and inheritance work a bit differently respect to other languages like C#. I tried to implement the
logic anyway and I have created an additional enemy, called boss, which shares part of the logic with the Enemy object.
Apparently it works, although I'm not 100% sure that a purist in javascript would find it properly implemented.
However my motto in these cases is a phrase of Theodore Roosevelt that said:
